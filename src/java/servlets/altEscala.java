 ...
 * @author matheus
 */
@WebServlet(name = "altEscala", urlPatterns = {"/views/altEscala"})
public class altEscala extends HttpServlet {
            ...
            if (!"".equals(tipo) && !"".equals(pesoMin) && !"".equals(pesoMax) && !"".equals(preco)) {
                ...
                for (Escala esc : escalas) {
                    if (doubPesoMin >= esc.getPsoMin() && doubPesoMax <= esc.getPesoMax() || doubPreco == esc.getPreco()) {
                        if (tipo.equals(esc.getTipo())) {
                            System.out.println(esc.getTipo());
                        } else {
                ...
                EscalaDAO.editEscala(E, idInt);

                request.setAttribute("msgRetorno", "<div class=\'col-md-12 alert alert-success\'>Escala alterada com sucesso!</div>\n");
                request.getRequestDispatcher("adm-admin-esca.jsp").forward(request, response);
