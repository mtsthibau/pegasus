 ...
 * @author matheus
 */
@WebServlet(name = "iniciarEntrega", urlPatterns = {"/views/iniciarEntrega"})
public class iniciarEntrega extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            email.addTo(p.getEmail(), p.getDestinatario());
            email.setFrom(e.getEmail(), e.getNome());
            email.addCc(r.getEmail(), r.getNome());
            email.setSubject("PEGASUS - ENTREGA INICIADA"); // assunto do e-mail
            // set the html message
            email.setHtmlMsg("<div style='color:#FF530D'><h2><strong>PEGASUS ENTREGAS</strong></h2><br></div>"
                    + "<div><h3>Sua entrega Nº" + p.getId() + " está a caminho, em breve o entregador estará no seu destino. Fique Atento!</h3><br></div>");
            email.send();
